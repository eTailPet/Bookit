...
public class PublicCatalogStepDef {
	...
	public void verify_user_is_able_to_search_an_item() throws Throwable {
		...
		for(WebElement item : pp.productName()) {
			...
			System.out.println("The entered item is found and testcase passed");
			}
			else {
				System.out.println("The entered item is not matching/found");
				driver.quit();
			}
		}
	}
	...
	public void verify_user_is_able_to_perform_the_actions_show_on_website_taxable_and_delivery_methods_on_an_item() throws Throwable {
		...
		pp.clickSearch().click();
		Thread.sleep(1000);
	//	boolean isChecked = pp.checkShowOnWebSite().isSelected();
	...
	public void verify_user_is_able_to_perform_the_actions_show_on_website_taxable_and_delivery_methods_on_list_of_items() throws Throwable {
		
		System.out.println("verify whether user is able to perform the actions show on website taxable and delivery methods on list of items");
		pp = new ProductsPage(driver);
		pp.clickCatalog().click();
		Thread.sleep(1000);
		pp.clickMyProducts().click();
		...
		pp.clickSearch().click();
		Thread.sleep(3000);
		for(WebElement checkbox : pp.verifyCheckboxes()) {
			//pp.tableColumnCheckboxes().get(i).click();
			//pp.tableColumnCheckboxes().get(i+1).click();
			checkbox.click();
		}
	...
	public void verify_user_is_able_to_click_on_clear_selection() throws Throwable {
		...
		Thread.sleep(1000);
		for(int i=0;i<pp.tableRow().size();i++) {
	...
	public void verify_user_is_able_to_reset_to_the_default_option() throws Throwable {
		...
		Assert.assertEquals(deliveryMethod,"Delivery Method");
		Assert.assertEquals(showOnWeb,"Show on website");
		Assert.assertEquals(brand, "-- Brand --");
		Assert.assertEquals(category, "Select Categories");
		Assert.assertEquals(store, "Etail (ID:2)");
		Assert.assertEquals(taxable, "Is Taxable");
	...
	public void verify_user_is_able_to_print_one_label() throws Throwable {
		
		System.out.println("Verify whether user is able to print one label of each selected item");
		pp = new ProductsPage(driver);
		pp.clickCatalog().click();
		Thread.sleep(1000);
		pp.clickMyProducts().click();
		...
		pp.clickHeaderCheckbox().click();
		Thread.sleep(1000);
		pp.clickPrintLabel().click();
		Thread.sleep(1000);
		pp.clickPrintOne().click();
		Thread.sleep(3000);
...
//		String totalCount = pp.getLabelCount().getText();
//		String[] firstvalue = totalCount.split("");
//		System.out.println(firstvalue[0]);
//		
	}
}
